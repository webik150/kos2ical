...
function generateICal() {
    ...
    let listky = $("#tabRozvrh [id^=detaillistek");
    let pozicniListky = $("#tabRozvrh [id^=listek");

    //Go through all of them and separate free text from text in tags
    for (let i = 0; i < listky.length; i++) {
        let separators = $(listky.get(i)).children().contents().filter(function() {
            return this.nodeType == 1;
        });
        let info = [];
        let nadpisy = $(listky.get(i)).children().contents().filter(function() {
        ...
        ev.startDate.day += pozicniListky[i].offsetTop / 120;
        ...
        ev.endDate.day += pozicniListky[i].offsetTop / 120;
		...
		//Configure weekly
        vevent.addPropertyWithValue('RRULE', 'FREQ=WEEKLY;BYDAY=' + dayFromPosition(pozicniListky[i]) + ';INTERVAL=1');
